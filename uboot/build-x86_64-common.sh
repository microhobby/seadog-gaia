...
function compileBootScript () {
	mkimage -A x86_64 -O linux -T script -C none \
...
if [ "$2" == "clean" ] || [ "$2" == "uboot" ]; then
    writeln "🧹 CLEAN"
    make CROSS_COMPILE=x86_64-linux-gnu- O=$artifacts clean
    checkError
fi

pwd
writeln "🧰 CONFIG"
make CROSS_COMPILE=x86_64-linux-gnu- O=$artifacts $defconfig
checkError

writeln "🔥 COMPILE"
make CROSS_COMPILE=x86_64-linux-gnu- O=$artifacts -j $jobs
checkError
cd -
